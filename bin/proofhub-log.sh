...
if [ ! -f "$proofhub_file" ]; then
 echo -e "$(tput bold)$(tput setaf 1)Error: $proofhub_file not found. Please run 'npm run proofhub:init'"$(tput sgr0)
 echo -e "Exiting..."
    exit 1
fi
...
todolist_response=$(curl -s --location "$api_endpoint/todolists" \
    --header "User-Agent: $user_agent" \
    --header "x-api-key: $api_key")
...
while read -r entry; do
    ...
    options+=("$id") 
    ((index++))
done < <(jq -c '.[] | {title: .title, id: .id}' <<< "$todolist_response")
...
# Subtract 1 from selected_index to get the correct array index
...
echo -e "$(tput bold)$(tput setaf 11) Select Proofhub Task"$(tput sgr0)
